...
\definecolor{gibberellin}{HTML}{D55E00}   % matches plot_npf_phylo_tree.py default
\definecolor{glucosinolate}{HTML}{009E73} % matches plot_npf_phylo_tree.py default
...
\setbeamertemplate{footline}{%
  ...
  \hbox{%
    \begin{beamercolorbox}[wd=\paperwidth, ht=2.6ex, dp=1.2ex, right, rightskip=0.3cm]{footlineright}
      \usebeamerfont{footline}\tiny CC BY 4.0~\raisebox{-0.15em}{\includegraphics[height=1.4ex]{by-4.0-88x31.pdf}}\hspace{0.4em}\insertframenumber\,/\,\inserttotalframenumber
    \end{beamercolorbox}%
  }%
}
...
\begin{frame}
  \begin{tikzpicture}[remember picture, overlay]
    \node[anchor=north] at ([yshift=-0.5cm]current page.north) {
      ...
      \hspace{1.4cm}%
      {\setlength{\fboxsep}{4pt}\colorbox{white}{\includegraphics[height=0.7cm]{ibmp-color.png}}}%
      \hspace{1.4cm}%
...
\begin{frame}{Nitrate and Peptide Transporters (NPF)}
  \begin{columns}[c]
    ...
    \begin{column}{0.52\textwidth}
      \centering
      \colorbox{white}{\includegraphics[width=0.76\textwidth]{NPF_tissue_distribution.jpg}}\\[0.1em]
...
\begin{frame}{NPF family: clade partitioning}
  \begin{columns}[T]
    \begin{column}{0.42\textwidth}
      \footnotesize
      \begin{itemize}
        \setlength{\itemsep}{0.3em}\setlength{\topsep}{0.1em}
        \item \textbf{53 NPF sequences} in \textit{Arabidopsis thaliana},
          aligned with MUSCLE
        \item Tree splits the family into \textbf{8 clades}
          (NPF1--NPF8 subfamily numbering)
        \item Highlighted in red: the best-characterized importers --
          NPF6.3/NRT1.1, NPF6.2/NRT1.4, NPF7.3/NRT1.5
        \item[$\Rightarrow$] Clade position gives a natural axis for
          comparing predicted conformations
      \end{itemize}
    \end{column}
    \begin{column}{0.55\textwidth}
      \centering
      \colorbox{white}{\includegraphics[width=0.66\textwidth]{NPFs_phylogenetic_tree.jpg}}\\[0.1em]
      {\tiny Phylogenetic tree of the NPF family (\textit{A.\ thaliana}), 53
      sequences aligned with MUSCLE; NPF6.3/NRT1.1, NPF6.2/NRT1.4 and
      NPF7.3/NRT1.5 marked in red.
      Morales de los R\'{i}os \textit{et al.}, \textit{Plant Physiology and
      Biochemistry} 168 (2021), CC BY-NC-ND 4.0.}
    \end{column}
  \end{columns}
\end{frame}

\begin{frame}{Clade membership $\neq$ shared substrate}
  \begin{columns}[T]
    \begin{column}{0.42\textwidth}
      \footnotesize
      \begin{itemize}
        \setlength{\itemsep}{0.3em}\setlength{\topsep}{0.1em}
        \item Our own tree, same 53 sequences, MUSCLE end to end
          (alignment + UPGMA, script: \texttt{plot\_npf\_phylo\_tree.py})
        \item \textcolor{gibberellin}{\textbf{Gibberellin importers}} (8
          proteins, GA1 substrate): scattered across NPF2/3/4 --
          \textbf{weak} clade colocalisation
        \item \textcolor{glucosinolate}{\textbf{Glucosinolate
          importers}}/GTR1--3: adjacent leaves, one NPF2 sub-clade --
          \textbf{strong} clade colocalisation
        \item[$\Rightarrow$] Shared substrate doesn't imply shared clade --
          homology alone can't stand in for per-protein prediction
      \end{itemize}
    \end{column}
    \begin{column}{0.55\textwidth}
      \centering
      \includegraphics[width=0.76\textwidth]{npf_clade_tree_transparent_slide.png}\\[0.2em]
      {\tiny Branch color = auto-detected clade (legend in
      \texttt{npf\_clade\_tree\_transparent.png})}
    \end{column}
  \end{columns}
\end{frame}
